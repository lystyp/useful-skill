...
| [isolated-worktree-session](skills/isolated-worktree-session/) | 平行多 session 改 code 時避免互相污染。**純手動：只有輸入 `/isolated-worktree-session` 才執行** —— 不自動觸發，也不會主動問你要不要啟用。呼叫後從 HEAD 切 temp branch + 開 worktree（開在 repo root 外的 sibling，雙向隔離）、嚴格鎖定 session 在 worktree 內、結束時詢問是否 cherry-pick 回原 branch。 |
